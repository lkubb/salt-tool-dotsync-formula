...
We will see a quick setup to configure the ``tool_dotsync`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_dotsync`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_dotsync`` formulas under ``/srv/salt/tool_dotsync/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_dotsync/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_dotsync/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_dotsync/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_dotsync/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_dotsync`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_dotsync`` formulas under ``/srv/salt/tool_dotsync/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_dotsync/parameters/roles
...
We create a configuration for the role ``tool_dotsync/server`` in ``/srv/salt/tool_dotsync/parameters/roles/tool_dotsync/server.yaml``:
...
We create another configuration for the role ``tool_dotsync/client`` in ``/srv/salt/tool_dotsync/parameters/roles/tool_dotsync/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_dotsync/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_dotsync/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_dotsync/parameters/dns:domain/  '/srv/salt/tool_dotsync/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-dotsync-as-json:
      test.nop:
        - name: {{ dotsync | json }}
...
.. code-block:: sls
    ...
    dotsync-config-file-file-managed:
      file.managed:
        - name: {{ dotsync.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dotsync-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ dotsync.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            dotsync: {{ dotsync | json }}

This ``sls`` file expose a ``dotsync`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dotsync['winner'] }}
